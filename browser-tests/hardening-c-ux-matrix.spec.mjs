...
test("Host challenge selector offers 3, 6, 9, and 12 and synchronizes the chosen target to Player Lobby", async ({
  ...
  const players = [];
  try {
    const selector = host.page.getByRole("radiogroup", { name: "عدد التحديات الأساسية" });
    ...
    players.push(player);
    await expect(player.page.getByText(/9 تحديات أساسية/)).toBeVisible();
    ...
    await expect(host.page.getByText("🏅 3 تحديات أساسية")).toBeVisible();
    await expect(player.page.getByText(/3 تحديات أساسية/)).toBeVisible();
    ...
    await expect(host.page.getByText("🏅 12 تحديات أساسية")).toBeVisible();
    await expect(player.page.getByText(/12 تحديات أساسية/)).toBeVisible();
