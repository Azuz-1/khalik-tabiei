...
async function expectRosterSize(ownerPage, size) {
  await expect(ownerPage.locator(".count-pill")).toContainText(String(size));
  await expect(ownerPage.locator(".seat-badge")).toHaveCount(size);
  const seats = await ownerPage.locator(".seat-badge").allTextContents();
  expect(new Set(seats).size).toBe(size);
}

test("Owner and Player rendered flows stay accessible at 3, 6, and 10 total players", async ({ browser }) => {
  const owner = await createOwner(browser);
  const players = [];
  try {
    // Owner already occupies one player slot, so nine joins fill the 10-player room.
    for (let index = 1; index <= 9; index += 1) {
      players.push(await joinPlayer(browser, owner.code, `لاعب${index}`));
      const total = index + 1;
      if ([3, 6, 10].includes(total)) await expectRosterSize(owner.page, total);
    }

    const mobileOverflow = await players[0].page.evaluate(() => document.documentElement.scrollWidth - window.innerWidth);
    expect(mobileOverflow).toBeLessThanOrEqual(1);

    const viewport = await owner.page.locator('meta[name="viewport"]').getAttribute("content");
    expect(viewport).not.toContain("maximum-scale");

    const closeRoom = owner.page.getByRole("button", { name: "إغلاق الغرفة" });
    await closeRoom.click();
    const closeDialog = owner.page.getByRole("dialog", { name: "إغلاق الغرفة؟" });
    await expect(closeDialog).toBeVisible();
    await expect(owner.page.locator("[data-app-content]")).toHaveAttribute("inert", "");
    await expect(owner.page.getByRole("button", { name: "إلغاء" })).toBeFocused();
    expect(await owner.page.getByRole("dialog").count()).toBe(1);
    await owner.page.keyboard.press("Tab");
    await expect(owner.page.getByRole("button", { name: "إغلاق الغرفة", exact: true }).last()).toBeFocused();
    await owner.page.keyboard.press("Escape");
    await expect(closeDialog).toBeHidden();
    await expect(closeRoom).toBeFocused();

    const stablePlayerName = "لاعب6";
    const stableChip = owner.page.locator(".chip", { hasText: stablePlayerName });
    ...
    await owner.page.locator("button.floating-players").click();
    const manager = owner.page.getByRole("dialog", { name: "اللاعبين" });
    ...
    await expect(owner.page.locator("[data-game-surface]")).toHaveAttribute("inert", "");

    const ownerRow = manager.locator(".manager-player-row", { hasText: "المالك" });
    await expect(ownerRow).toContainText("مالك الغرفة");
    await expect(ownerRow.getByRole("button", { name: "إخراج" })).toHaveCount(0);

    const offlineRow = manager.locator(".manager-player-row", { hasText: "لاعب5" });
    await offlineRow.getByRole("button", { name: "إخراج" }).click();
    const kickDialog = owner.page.getByRole("dialog", { name: "إخراج لاعب5؟" });
    ...
    const stableSeatAfter = (await owner.page.locator(".chip", { hasText: stablePlayerName }).locator(".seat-badge").textContent())?.trim();
...
test("Unicode names, stale confirmations, Player leave, focus restoration, and reduced motion work in browser", async ({ browser }) => {
  ...
  try {
    const salem = await joinPlayer(browser, owner.code, "سالم");
    ...
    await duplicate.goto(`/join/${owner.code}`);
    ...
    const staleDialog = owner.page.getByRole("dialog", { name: "إخراج سالم؟" });
    ...
    await expect(salem.page.getByRole("button", { name: "سوّ غرفة والعب معنا" })).toBeVisible();
    ...
    await expect(emoji.page.getByRole("button", { name: "سوّ غرفة والعب معنا" })).toBeVisible();
