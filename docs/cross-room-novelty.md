...
- `usedPromptIds` remains the authoritative exact no-repeat mechanism inside the current game.
- If every otherwise-valid candidate appears seen (including an all-ones malicious filter), the picker falls back to the normal current-game pool. Novelty data can never block gameplay or alter scoring, voting, roles or timers.
...
For v1 the filter uses 24,576 bits and 18 hash positions. At 900 inserted prompts, the modeled false-positive probability is about 0.000203%. A false positive can make an unseen prompt temporarily look seen; it cannot make a recorded prompt look unseen.
